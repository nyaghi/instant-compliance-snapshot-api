...
exports.setup=({origin='https://staging.compliance-express.com',clock={},source}={})=>{
 ...
 const context={location:{origin},document:{querySelector:()=>main,createElement:()=>new Element(),head:new Element(),getElementById:id=>elements[id]},window:{dispatchEvent:e=>events.push(e)},CustomEvent:class{constructor(type,x){this.type=type;this.detail=x.detail;}},performance,setInterval,clearInterval,setTimeout,clearTimeout,AbortController,...clock,internalUnlocked:true,isComplianceExpressEmail:()=>true,API_BASE:'https://staging.example',requestSingleState:(...args)=>{calls.push(args);peak=Math.max(peak,++active);return new Promise((resolve,reject)=>pending.set(args[3],{resolve,reject}));}};
 vm.createContext(context);vm.runInContext(source||fs.readFileSync(path.join(__dirname,'../web-staging/sales-mode.js'),'utf8'),context);

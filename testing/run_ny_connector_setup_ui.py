...
OUT=WORK.parents[1]/'outputs/ny-search-diagnostics-20260911/setup-ui'
...
class SetupUI(unittest.TestCase):
    ...
    def page(self,ready,guide=False,legacy=False,width=1280,outdated=False):
        ...
        });'''.replace('READY',json.dumps(ready)).replace('VERSION',json.dumps('0.1.2' if outdated else '0.1.4')).replace('CAPABILITIES',json.dumps(['lookup-tab-v1','verification-retry-v1'] if outdated else ['lookup-tab-v1','verification-retry-v1','search-verification-retry-v1','search-schema-errors-v1'])))
        def route(r):

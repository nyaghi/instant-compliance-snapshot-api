...
async function setup(names,ny=null){
 ...
 const window={dispatchEvent(){},CCIdentityConfig:()=>({email:'qa@example.test',admin_passcode:'test-only',apiBase:'https://staging.example'}),CCNYConnector:{lookup:async()=>ny}};
...
test('an independent NY exact-EIN alias restores selection',async()=>{
 const ny={identity:{complete:true,names:[{name:conflict.name,verified:true,evidence:[{source:'NY',type:'Registered name'}]}]}};
 const {ids,window}=await setup([conflict],ny);assert.equal(ids.alternateNameList.children[0].children[0].children[0].checked,true);assert.equal(window.CCIdentity.names()[0],conflict.name);
});

...
class ArkansasCompletionTests(unittest.TestCase):
    ...
    def lookup(self, overrides=None, stale=False, aliases=None):
        if aliases is not None:c.REVIEWED_NAME_CONTEXT.set({EIN:tuple(aliases)})
        page=MagicMock(); submitted=[];query=[''];overrides=overrides or {}
        def fill(value):
        ...
        with patch.object(c,'ar_wait_for_search_form',return_value=True),patch.object(c,'registry_page_body',side_effect=body),patch.object(c,'ar_result_rows',side_effect=rows),patch.object(c,'safe_wait_for_network_idle'):
            result=c.search_ar_precise(page,c.checker.Organization(NAME,EIN))
    ...
    def test_real_case_completes_without_full_name_or_covered_punctuation_retries(self):
        ...
        self.assertEqual(queries,[NAME,'EVERY WOMAN TREATY, INC.','Every Woman',PREFIX,'Every-Woman Treaty','Treaty']);self.assertEqual(c.public_status(r),'Not Registered');self.assertTrue(r.success)
    ...
    def test_empty_suffixed_search_does_not_skip_shorter_or_comma_spelling(self):
        ...
        try:
            ...
            r,queries=self.lookup({'CHRISTIAN RESEARCH INSTITUTE, INC.':{'body':'Back to Search Form Registration Date','rows':[row]}},aliases=['CHRISTIAN RESEARCH INSTITUTE, INC.'])
    ...
    def test_blocked_prefix_cannot_certify_negative(self):
        ...
        self.assertEqual(r.queries_attempted,[NAME,'EVERY WOMAN TREATY, INC.','Every Woman',PREFIX])

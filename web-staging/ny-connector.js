...
(() => {
  ...
  async function setup() {
    ...
    const ready = !!compatible(response) && response.capabilities?.includes("il-ga-public-dom-v1") && response.capabilities?.includes("ga-exempt-record-v1"), update = !!response.ok && !ready;
    box.hidden = false;
  ...
  async function performLookup({ organization_name, ein, email, admin_passcode, device_id, onProgress, alternate_names, purpose = "registration", state: registryState = "NY", signal }) {
    ...
    const supported = c => compatible(c) && (registryState === "NY" || c.capabilities?.includes("il-ga-public-dom-v1")) && (registryState !== "GA" || c.capabilities?.includes("ga-exempt-record-v1"));
    if (refreshing) {

...
(() => {
  ...
  async function performLookup({ organization_name, ein, email, admin_passcode, device_id, onProgress, alternate_names, purpose = "registration", state: registryState = "NY", signal }) {
    ...
    const supported = c => compatible(c) && (registryState === "NY" || c.capabilities?.includes("il-ga-public-dom-v1") && c.capabilities?.includes("il-ga-complete-search-v2") && c.capabilities?.includes("il-session-reuse-v1") && c.capabilities?.includes("il-large-pages-v1")) && (registryState !== "GA" || c.capabilities?.includes("ga-exempt-record-v1") && c.capabilities?.includes("ga-legacy-rows-v1"));
    if (refreshing) {
